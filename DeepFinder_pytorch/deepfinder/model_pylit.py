...
class DeepFinder_model(pl.LightningModule):
    ...
    def initialize_seeds(self, x, sample_pos, num_seeds=100, margin=2):
        ...
        while len(all_seeds) < num_seeds:
            ...
            if len(shape) == 5:
                z_comp = np.random.uniform(margin, s_max3 - margin)
    ...
    def initialize_coords(self, x):
        ...
        if len(shape) == 5:
            ...
            coordinate_grid = torch.cat((grid_x.unsqueeze(2), grid_y.unsqueeze(2)), dim=2)
    ...
    def forward(self, x, gt_centers):
        ...
        for i in range(x.shape[0]):
            seeds = self.initialize_seeds(x, gt_centers[i], num_seeds=64, margin=2.)
            if i == 0:
                seeds0 = seeds.clone()
    ...
    def eval_plots(self, batch_target, pred, mins_seeds, seeds, img, batch_data, gt_img):
        ...
        plt.colorbar()
        plt.savefig('/Users/lorenz.lamm/PhD_projects/DeepFinder_MeanShift/log_imgs/loss_vals.png')
        ...
        # if points.shape[0] > 8:  # and points.shape[0] < img[0,0].shape[1] * img[0,0].shape[0]:
            ...
            # center_coords, center_idcs = ms.mean_shift(points, ms_weights, weighting='simple', n_pr=1, fuse_dist=2.)
        ...
        plt.imshow(batch_data.clone().detach().cpu()[0, 0, :, :, 25])
        plt.subplot(1, 4, 2)
        ...
        plt.imshow(img.clone().detach().cpu()[0, 0, :, :, 25])
        plt.colorbar(fraction=0.046, pad=0.04)
        plt.subplot(1, 4, 3)
        plt.imshow(ms_img[:,:,25])
        # plt.scatter(x = points[:, 1], y=points[:,0])
        plt.colorbar(fraction=0.046, pad=0.04)
        plt.subplot(1,4,4)
        plt.imshow(gt_img[:,:,25])
